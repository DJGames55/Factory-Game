...
public class Interact : MonoBehaviour
{
    ...
    private void Update()
    {
        if (!isBuilding) 
        {
            if (heldObject != null)
            {
                ...
                heldObject.transform.SetPositionAndRotation(holdPos.transform.position, holdPos.transform.rotation);
            }
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (!isBuilding)
        {
            Debug.Log("Matching collider detected: " + other.gameObject.name);

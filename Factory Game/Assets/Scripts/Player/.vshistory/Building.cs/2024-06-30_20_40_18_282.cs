...
public class Building : MonoBehaviour
{
    ...
    private void SetBluePrint(GameObject blueprintObject)
    {
        ...
        blueprintObject.layer = holdLayer;
    ...
    public void StopPlacingObject(GameObject bpO)
    {
        Destroy(bpO);
        _interact.HoldingCameraStop();
    }

    private void PlaceObject(GameObject bpO)
    {
    ...
    private void UnableToPlace(GameObject bpO)
    {
        _interact.HoldingCameraStop();

        // Change Material
        Renderer renderer = bpO.GetComponent<Renderer>();
        if (renderer != null)
        {
            renderer.material = blueprintMaterial;
        }
        ...
        // Change Children Materal
        if (bpO.transform.childCount != 0)
        {
            foreach (Transform child in bpO.transform)
            {
                Renderer childRenderer = child.GetComponent<Renderer>();
                if (childRenderer != null)
                {

...
public class PlayerControls : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        sensitivity = _gameManager.GetComponent<GameManager>().sens * 10;
    }
    ...
    private void Look()
    {
        ...
        head.transform.rotation = Quaternion.Euler(targetRotationY, 0f, 0f);
        targetRotationY = Mathf.Repeat(targetRotationY, 90f);

        float targetRotationFace = head.transform.rotation.eulerAngles.x - lookDirection.y * sensitivity * Time.deltaTime;
        if (targetRotationFace <= 0f)
        {
            face.transform.rotation = Quaternion.Euler(0f, 0f, 0f);
        }
        else
        {
            face.transform.rotation = Quaternion.Euler(targetRotationFace, 0f, 0f);
        }
        targetRotationFace = Mathf.Repeat(targetRotationFace, 90f);
    }
